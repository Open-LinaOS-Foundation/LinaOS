...
static struct linaos_prom64_registers pavail[MAX_BANKS];
...
{
	const struct linaos_prom64_registers *x = a, *y = b;
...
static void __init read_obp_memory(const char *property,
				   struct linaos_prom64_registers *regs,
				   int *num_ents)
{
	...
	ents = prop_size / sizeof(struct linaos_prom64_registers);
	if (ents > MAX_BANKS) {
	...
	*num_ents = ents;
	...
	sort(regs, ents, sizeof(struct linaos_prom64_registers),
	     cmp_p64, NULL);
}
...
struct linaos_prom_translation prom_trans[512] __read_mostly;
...
{
	const struct linaos_prom_translation *x = a, *y = b;
...
static void __init read_obp_translations(void)
{
	...
	n = n / sizeof(struct linaos_prom_translation);

	ents = n;

	sort(prom_trans, ents, sizeof(struct linaos_prom_translation),
	     cmp_ptrans, NULL);
	...
	for (i = 0; i < (last - first); i++) {
		struct linaos_prom_translation *src = &prom_trans[i + first];
		struct linaos_prom_translation *dest = &prom_trans[i];
...
static struct linaos_prom64_registers pall[MAX_BANKS] __initdata;
 ...
 * vmlinaos and initrd. To be more precise the initrd size could be used to
...
void free_initmem(void)
{
	...
	/*
	 * The init section is aligned to 8k in vmlinaos.lds. Page align for >8k pagesizes.
	 */
