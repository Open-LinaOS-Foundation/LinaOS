 ...
 */
static int pcic0_up;
static struct linaos_pcic pcic0;
...
{
	struct linaos_pcic *pcic;
	struct linaos_prom_registers regs[PROMREG_MAX];
	struct linaos_pbm_info* pbm;
	char namebuf[64];
	phandle node;
	int err;
...
static void __init pcic_pbm_scan_bus(struct linaos_pcic *pcic)
{
	struct linaos_pbm_info *pbm = &pcic->pbm;
...
static int pdev_to_pnode(struct linaos_pbm_info *pbm, struct pci_dev *pdev)
{
	struct linaos_prom_pci_registers regs[PROMREG_MAX];
	int err;
...
static void pcic_map_pci_device(struct linaos_pcic *pcic,
...
static void
pcic_fill_irq(struct linaos_pcic *pcic, struct pci_dev *dev, int node)
{
