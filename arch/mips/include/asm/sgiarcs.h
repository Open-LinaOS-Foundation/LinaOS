...
enum linaos_identifier {
	bogus, ronly, removable, consin, consout, input, output
};

/* A prom device tree component. */
struct linaos_component {
	enum linaos_devclass	class;	/* node class */
	enum linaos_devtypes	type;	/* node type */
	enum linaos_identifier	iflags; /* node flags */
...
typedef struct linaos_component pcomponent;
...
union linaos_memtypes {
    enum arcs_memtypes arcs;
    enum arc_memtypes arc;
};

struct linaos_mdesc {
	union linaos_memtypes type;
	ULONG base;
	ULONG pages;
};
...
enum linaos_seekmode {
	absolute, relative
};

enum linaos_mountops {
	media_load, media_unload
};

/* This prom has a bolixed design. */
struct linaos_bigint {
#ifdef __MIPSEL__
	u32 lo;
	s32 hi;
#else /* !(__MIPSEL__) */
...
struct linaos_finfo {
	struct linaos_bigint   begin;
	struct linaos_bigint   end;
	struct linaos_bigint   cur;
	enum linaos_devtypes   dtype;
...
#define ROMVECTOR ((struct linaos_romvec *) (long)(PROMBLOCK)->romvec)

/* Cache layout parameter block. */
union linaos_cache_key {
	struct param {
#ifdef __MIPSEL__
		unsigned short size;
		unsigned char lsize;
