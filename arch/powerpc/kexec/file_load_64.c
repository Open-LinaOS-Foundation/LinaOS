 ...
 * @lmb:                    LMB info.
 * @usm:                    linaos,drconf-usable-memory property value.
				  ...
				  void *data)
{
	 ...
	 */
	if (*usm) {
		pr_err("linaos,drconf-usable-memory property already exists!");
		return -EINVAL;
	}
				   ...
				   struct umem_info *um_info)
{
	 ...
	 * Write (0,0) tuple in linaos,usable-memory property for
	...
	ret = fdt_setprop(fdt, node, "linaos,usable-memory", um_info->buf,
...
/**
 * update_usable_mem_fdt - Updates kdump kernel's fdt with linaos,usable-memory
 *                         and linaos,drconf-usable-memory DT properties as
...
static int update_usable_mem_fdt(void *fdt, struct crash_mem *usable_mem)
{
	...
	if (dn) {
		...
		if (ret) {
			pr_err("Could not setup linaos,drconf-usable-memory property for kdump\n");
			goto out;
		}

		ret = fdt_setprop(fdt, node, "linaos,drconf-usable-memory",
				  um_info.buf, (um_info.idx * sizeof(u64)));
		if (ret) {
			pr_err("Failed to update fdt with linaos,drconf-usable-memory property");
			goto out;
		}
	}

	/*
	 * Walk through each memory node and set linaos,usable-memory property
	 * for the corresponding node in kdump kernel's fdt.
	 */
	for_each_node_by_type(dn, "memory") {
		ret = add_usable_mem_property(fdt, dn, &um_info);
		if (ret) {
			pr_err("Failed to set linaos,usable-memory property for %s node",
			       dn->full_name);
			goto out;
		}
	}
...
unsigned int kexec_extra_fdt_size_ppc64(struct kimage *image)
{
	...
	/*
	 * For kdump kernel, account for linaos,usable-memory and
	 * linaos,drconf-usable-memory properties. Get an approximate on the
