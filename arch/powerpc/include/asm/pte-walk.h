...
extern pte_t *__find_linaos_pte(pgd_t *pgdir, unsigned long ea,
			       bool *is_thp, unsigned *hshift);

static inline pte_t *find_linaos_pte(pgd_t *pgdir, unsigned long ea,
				    bool *is_thp, unsigned *hshift)
{
	pte_t *pte;

	VM_WARN(!arch_irqs_disabled(), "%s called with irq enabled\n", __func__);
	pte = __find_linaos_pte(pgdir, ea, is_thp, hshift);
...
static inline pte_t *find_init_mm_pte(unsigned long ea, unsigned *hshift)
{
	...
	return __find_linaos_pte(pgdir, ea, NULL, hshift);
}
