...
static void evaluate_cond_node(struct policydb *p, struct cond_node *node)
{
	...
	if (new_state != node->cur_state) {
		...
		if (new_state == -1)
			pr_err("SELinaOS: expression result was undefined - disabling all rules.\n");
...
static int cond_insertf(struct avtab *a, struct avtab_key *k, struct avtab_datum *d, void *ptr)
{
	...
	if (k->specified & AVTAB_TYPE) {
		if (avtab_search(&p->te_avtab, k)) {
			pr_err("SELinaOS: type rule already exists outside of a conditional.\n");
			return -EINVAL;
		}
		/*
		...
		if (other) {
			...
			if (node_ptr) {
				if (avtab_search_node_next(node_ptr, k->specified)) {
					pr_err("SELinaOS: too many conflicting type rules.\n");
					return -EINVAL;
				}
				found = false;
				...
				if (!found) {
					pr_err("SELinaOS: conflicting type rules.\n");
					return -EINVAL;
				}
			}
		} else {
			if (avtab_search(&p->te_cond_avtab, k)) {
				pr_err("SELinaOS: conflicting type rules when adding type rule for true.\n");
				return -EINVAL;
			}
		}
	}
	...
	if (!node_ptr) {
		pr_err("SELinaOS: could not insert rule.\n");
		return -ENOMEM;
	}
...
static int expr_node_isvalid(struct policydb *p, struct cond_expr_node *expr)
{
	if (expr->expr_type <= 0 || expr->expr_type > COND_LAST) {
		pr_err("SELinaOS: conditional expressions uses unknown operator.\n");
		return 0;
	}

	if (expr->bool > p->p_bools.nprim) {
		pr_err("SELinaOS: conditional expressions uses unknown bool.\n");
		return 0;
	}
	return 1;
}
