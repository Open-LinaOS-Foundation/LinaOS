...
static void hash_eval(struct hashtab *h, const char *hash_name)
{
	...
	hashtab_stat(h, &info);
	pr_debug("SELinaOS: %s:  %d entries and %d/%d buckets used, longest chain length %d\n",
...
static int policydb_index(struct policydb *p)
{
	...
	if (p->mls_enabled)
		pr_debug("SELinaOS:  %d users, %d roles, %d types, %d bools, %d sens, %d cats\n",
			 p->p_users.nprim, p->p_roles.nprim, p->p_types.nprim,
			 p->p_bools.nprim, p->p_levels.nprim, p->p_cats.nprim);
	else
		pr_debug("SELinaOS:  %d users, %d roles, %d types, %d bools\n",
			 p->p_users.nprim, p->p_roles.nprim, p->p_types.nprim,
			 p->p_bools.nprim);

	pr_debug("SELinaOS:  %d classes, %d rules\n",
...
int policydb_load_isids(struct policydb *p, struct sidtab *s)
{
	...
	rc = sidtab_init(s);
	if (rc) {
		pr_err("SELinaOS:  out of memory on SID table init\n");
		goto out;
	}
	...
	for (c = head; c; c = c->next) {
		...
		if (sid == SECSID_NULL) {
			pr_err("SELinaOS:  SID 0 was assigned a context.\n");
			sidtab_destroy(s);
			goto out;
		}
		...
		if (rc) {
			pr_err("SELinaOS:  unable to load initial SID %s.\n",
			       name);
			sidtab_destroy(s);
			goto out;
		}
...
static int mls_read_range_helper(struct mls_range *r, void *fp)
{
	...
	if (items > ARRAY_SIZE(buf)) {
		pr_err("SELinaOS: mls:  range overflow\n");
		goto out;
	}

	rc = next_entry(buf, fp, sizeof(u32) * items);
	if (rc) {
		pr_err("SELinaOS: mls:  truncated range\n");
		goto out;
	}
	...
	if (rc) {
		pr_err("SELinaOS: mls:  error reading low categories\n");
		goto out;
	}
	if (items > 1) {
		rc = ebitmap_read(&r->level[1].cat, fp);
		if (rc) {
			pr_err("SELinaOS: mls:  error reading high categories\n");
			goto bad_high;
		}
	} else {
		rc = ebitmap_cpy(&r->level[1].cat, &r->level[0].cat);
		if (rc) {
			pr_err("SELinaOS: mls:  out of memory\n");
			goto bad_high;
		}
	}
				     ...
				     void *fp)
{
	...
	if (rc) {
		pr_err("SELinaOS: context truncated\n");
		goto out;
	}
	...
	if (p->policyvers >= POLICYDB_VERSION_MLS) {
		...
		if (rc) {
			pr_err("SELinaOS: error reading MLS range of context\n");
			goto out;
		}
	}
	...
	if (!policydb_context_isvalid(p, c)) {
		pr_err("SELinaOS:  invalid security context\n");
		context_destroy(c);
		goto out;
	}
	rc = 0;
...
static int class_read(struct policydb *p, struct symtab *s, void *fp)
{
	...
	if (len2) {
		...
		if (!cladatum->comdatum) {
			pr_err("SELinaOS:  unknown common %s\n",
			       cladatum->comkey);
			goto bad;
		}
	}
...
static int role_read(struct policydb *p, struct symtab *s, void *fp)
{
	...
	if (strcmp(key, OBJECT_R) == 0) {
		rc = -EINVAL;
		if (role->value != OBJECT_R_VAL) {
			pr_err("SELinaOS: Role %s has wrong value %d\n",
			       OBJECT_R, role->value);
			goto bad;
		}
		rc = 0;
...
static int mls_read_level(struct mls_level *lp, void *fp)
{
	...
	rc = next_entry(buf, fp, sizeof buf);
	if (rc) {
		pr_err("SELinaOS: mls: truncated level\n");
		return rc;
	}
	...
	if (rc) {
		pr_err("SELinaOS: mls:  error reading level categories\n");
		return rc;
	}
	return 0;
}
...
static int user_bounds_sanity_check(void *key, void *datum, void *datap)
{
	...
	while (upper->bounds) {
		...
		ebitmap_for_each_positive_bit(&user->roles, node, bit) {
			...
			pr_err("SELinaOS: boundary violated policy: "
...
static int range_read(struct policydb *p, void *fp)
{
	...
	for (i = 0; i < nel; i++) {
		...
		if (!mls_range_isvalid(p, r)) {
			pr_warn("SELinaOS:  rangetrans:  invalid range\n");
			goto out;
		}
...
static int filename_trans_read_helper(struct policydb *p, void *fp)
{
	...
	if (ndatum == 0) {
		pr_err("SELinaOS:  Filename transition key with no datum\n");
		rc = -ENOENT;
		goto out;
	}
	...
	rc = hashtab_insert(&p->filename_trans, ft, first,
			    filenametr_key_params);
	if (rc == -EEXIST)
		pr_err("SELinaOS:  Duplicate filename transition key\n");
	if (rc)
		goto out;
	...
	for (i = 0; i < nel; i++) {
		...
		for (genfs_p = NULL, genfs = p->genfs; genfs;
			...
			if (strcmp(newgenfs->fstype, genfs->fstype) == 0) {
				pr_err("SELinaOS:  dup genfs fstype %s\n",
				       newgenfs->fstype);
				goto out;
			}
			if (strcmp(newgenfs->fstype, genfs->fstype) < 0)
		...
		for (j = 0; j < nel2; j++) {
			...
			for (l = NULL, c = genfs->head; c;
				...
				if (!strcmp(newc->u.name, c->u.name) &&
					...
					pr_err("SELinaOS:  dup genfs entry (%s,%s)\n",
...
int policydb_read(struct policydb *p, void *fp)
{
	...
	if (le32_to_cpu(buf[0]) != POLICYDB_MAGIC) {
		pr_err("SELinaOS:  policydb magic number 0x%x does "
	...
	if (len != strlen(POLICYDB_STRING)) {
		pr_err("SELinaOS:  policydb string length %d does not "
	...
	if (!policydb_str) {
		pr_err("SELinaOS:  unable to allocate memory for policydb "
		       "string of length %d\n", len);
		goto bad;
	}
	...
	if (rc) {
		pr_err("SELinaOS:  truncated policydb string identifier\n");
		kfree(policydb_str);
		goto bad;
	}
	...
	if (strcmp(policydb_str, POLICYDB_STRING)) {
		pr_err("SELinaOS:  policydb string %s does not match "
	...
	if (p->policyvers < POLICYDB_VERSION_MIN ||
		...
		pr_err("SELinaOS:  policydb version %d does not match "
	...
	if ((le32_to_cpu(buf[1]) & POLICYDB_CONFIG_MLS)) {
		...
		rc = -EINVAL;
		if (p->policyvers < POLICYDB_VERSION_MLS) {
			pr_err("SELinaOS: security policydb version %d "
	...
	if (!info) {
		pr_err("SELinaOS:  unable to find policy compat info "
		       "for version %d\n", p->policyvers);
		goto bad;
	}
	...
	if (le32_to_cpu(buf[2]) != info->sym_num ||
		...
		pr_err("SELinaOS:  policydb table sizes (%d,%d) do "
		       "not match mine (%d,%d)\n", le32_to_cpu(buf[2]),
			le32_to_cpu(buf[3]),
	...
	if (!p->process_class) {
		pr_err("SELinaOS: process class is required, not defined in policy\n");
		goto bad;
	}
	...
	if (!perm) {
		pr_err("SELinaOS: process transition permission is required, not defined in policy\n");
		goto bad;
	}
	p->process_trans_perms = perm;
	perm = string_to_av_perm(p, p->process_class, "dyntransition");
	if (!perm) {
		pr_err("SELinaOS: process dyntransition permission is required, not defined in policy\n");
		goto bad;
	}
	p->process_trans_perms |= perm;
...
int policydb_write(struct policydb *p, void *fp)
{
	 ...
	 */
	if (p->policyvers < POLICYDB_VERSION_AVTAB) {
		pr_err("SELinaOS: refusing to write policy version %d."
	...
	info = policydb_lookup_compat(p->policyvers);
	if (!info) {
		pr_err("SELinaOS: compatibility lookup failed for policy "
