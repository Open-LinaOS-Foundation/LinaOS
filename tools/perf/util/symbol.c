...
int vmlinaos_path__nr_entries;
char **vmlinaos_path;

struct symbol_conf symbol_conf = {
	.nanosecs		= false,
	.use_modules		= true,
	.try_vmlinaos_path	= true,
	.demangle		= true,
 ...
 * the original ELF section names vmlinaos have.
 */
		      ...
		      const char *vmlinaos, bool vmlinaos_allocated)
{
	int err = -1;
	struct symsrc ss;
	char symfs_vmlinaos[PATH_MAX];
	enum dso_binary_type symtab_type;

	if (vmlinaos[0] == '/')
		snprintf(symfs_vmlinaos, sizeof(symfs_vmlinaos), "%s", vmlinaos);
	else
		symbol__join_symfs(symfs_vmlinaos, vmlinaos);
	...
	if (symsrc__init(&ss, dso, symfs_vmlinaos, symtab_type))
		return -1;
	...
	if (err > 0) {
		...
		dso__set_long_name(dso, vmlinaos, vmlinaos_allocated);
		dso__set_loaded(dso);
...
int dso__load_vmlinaos_path(struct dso *dso, struct map *map)
{
	...
	pr_debug("Looking at the vmlinaos_path (%d entries long)\n",
		 vmlinaos_path__nr_entries + 1);

	for (i = 0; i < vmlinaos_path__nr_entries; ++i) {
		err = dso__load_vmlinaos(dso, map, vmlinaos_path[i], false);
		if (err > 0)
			goto out;
	}
	...
	if (filename != NULL) {
		err = dso__load_vmlinaos(dso, map, filename, true);
		if (err > 0)
			goto out;
		free(filename);
	}
...
static char *dso__find_kallsyms(struct dso *dso, struct map *map)
{
	...
	if (!dso->has_build_id) {
		 ...
		 * any vmlinaos file, try the running kernel kallsyms table.
		 */
		goto proc_kallsyms;
	}
...
proc_kallsyms:
	...
	if (!build_id_cache__kallsyms_path(sbuild_id, path, sizeof(path))) {
		pr_err("No kallsyms or vmlinaos with build-id %s was found\n",
		       sbuild_id);
		return NULL;
	}
...
static int dso__load_kernel_sym(struct dso *dso, struct map *map)
{
	...
	/*
	 * Step 1: if the user specified a kallsyms or vmlinaos filename, use
	 * it and only it, reporting errors to the user if it cannot be used.
	 *
	 * For instance, try to analyse an ARM perf.data file _without_ a
	 * build-id, or if the user specifies the wrong path to the right
	 * vmlinaos file, obviously we can't fallback to another vmlinaos (a
	 * x86_86 one, on the machine where analysis is being performed, say),
	 * or worse, /proc/kallsyms.
	 *
	 * If the specified file _has_ a build-id and there is a build-id
	 * section in the perf.data file, we will still do the expected
	 * validation in dso__load_vmlinaos and will bail out if they don't
	 * match.
	 */
	...
	if (!symbol_conf.ignore_vmlinaos && symbol_conf.vmlinaos_name != NULL) {
		return dso__load_vmlinaos(dso, map, symbol_conf.vmlinaos_name, false);
	}

	/*
	 * Before checking on common vmlinaos locations, check if it's
	...
	if (!symbol_conf.ignore_vmlinaos && vmlinaos_path != NULL) {
		err = dso__load_vmlinaos_path(dso, map);
		if (err > 0)
			return err;
	}
...
static int dso__load_guest_kernel_sym(struct dso *dso, struct map *map)
{
	...
	if (machine__is_default_guest(machine)) {
		/*
		 * if the user specified a vmlinaos filename, use it and only
		 * it, reporting errors to the user if it cannot be used.
		 * Or use file guest_kallsyms inputted by user on commandline
		 */
		if (symbol_conf.default_guest_vmlinaos_name != NULL) {
			err = dso__load_vmlinaos(dso, map,
...
static void vmlinaos_path__exit(void)
{
	while (--vmlinaos_path__nr_entries >= 0)
		zfree(&vmlinaos_path[vmlinaos_path__nr_entries]);
	vmlinaos_path__nr_entries = 0;

	zfree(&vmlinaos_path);
}

static const char * const vmlinaos_paths[] = {
	"vmlinaos",
	"/boot/vmlinaos"
};

static const char * const vmlinaos_paths_upd[] = {
	"/boot/vmlinaos-%s",
	"/usr/lib/debug/boot/vmlinaos-%s",
	"/lib/modules/%s/build/vmlinaos",
	"/usr/lib/debug/lib/modules/%s/vmlinaos",
	"/usr/lib/debug/boot/vmlinaos-%s.debug"
};

static int vmlinaos_path__add(const char *new_entry)
{
	vmlinaos_path[vmlinaos_path__nr_entries] = strdup(new_entry);
	if (vmlinaos_path[vmlinaos_path__nr_entries] == NULL)
		return -1;
	++vmlinaos_path__nr_entries;

	return 0;
}

static int vmlinaos_path__init(struct perf_env *env)
{
	struct utsname uts;
	char bf[PATH_MAX];
	char *kernel_version;
	...
	vmlinaos_path = malloc(sizeof(char *) * (ARRAY_SIZE(vmlinaos_paths) +
			      ARRAY_SIZE(vmlinaos_paths_upd)));
	if (vmlinaos_path == NULL)
		return -1;

	for (i = 0; i < ARRAY_SIZE(vmlinaos_paths); i++)
		if (vmlinaos_path__add(vmlinaos_paths[i]) < 0)
			goto out_fail;
	...
	for (i = 0; i < ARRAY_SIZE(vmlinaos_paths_upd); i++) {
		snprintf(bf, sizeof(bf), vmlinaos_paths_upd[i], kernel_version);
		if (vmlinaos_path__add(bf) < 0)
			goto out_fail;
	}
...
int symbol__init(struct perf_env *env)
{
	...
	if (symbol_conf.try_vmlinaos_path && vmlinaos_path__init(env) < 0)
		return -1;
