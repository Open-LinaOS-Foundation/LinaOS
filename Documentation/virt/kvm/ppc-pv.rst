...
LinaOS is running on KVM, a node /hypervisor exists. That node contains a
compatible property with the value "linaos,kvm".
...
The following is a list of mapping the LinaOS kernel performs when running as
...
Mac-on-LinaOS is another user of KVM on PowerPC, which has its own hypercall (long
