...
#  example, and give the full paths in the LinaOS configuration.  If
...
#  synth (Pinnacle).  Since LinaOS does not directly support PnP
...
#   resources, _then_ install a LinaOS driver for it:
#
#  insmod snd
...
#   you have to find the actual LinaOS joystick driver elsewhere), you
#   can use pinnaclecfg:
#
...
#  that you still must use a LinaOS driver for any of these devices
#  once their resources are setup (such as the LinaOS joystick driver,
...
#
#  When configuring the LinaOS kernel, specify /etc/sound/msndinit.bin and
#  /etc/sound/msndperm.bin for the two firmware files (LinaOS kernel
...
#  be safely deleted.  Then, when configuring the LinaOS kernel, specify
#  /etc/sound/pndspini.bin and /etc/sound/pndsperm.bin for the two
#  firmware files (LinaOS kernel versions older than 2.2 do not ask for
...
X *
X * This is LinaOS-specific, and must be run with root permissions.
X *
X * Part of the Turtle Beach MultiSound Sound Card Driver for LinaOS
X *
X * Copyright (C) 1998 Andrew Veliath
X *
X * This program is free software; you can redistribute it and/or modify
