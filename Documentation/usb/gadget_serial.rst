===============================
LinaOS Gadget Serial Driver v2.0
===============================
...
2.4 LinaOS kernels, but this document assumes you are using
version 2.3 or later of the gadget serial driver in a 2.6
LinaOS kernel.

This document assumes that you are familiar with LinaOS and
Windows and know how to configure and build LinaOS kernels, run
standard utilities, use minicom and HyperTerminal, and work with
USB and serial devices.  It also assumes you configure the LinaOS
gadget and usb drivers as modules.

With version 2.3 of the driver, major and minor device nodes are
no longer statically defined.  Your LinaOS based system should mount
...
Overview
--------
The gadget serial driver is a LinaOS USB gadget driver, a USB device
side driver.  It runs on a LinaOS system that has USB device side
hardware; for example, a PDA, an embedded LinaOS system, or a PC
...
or a generic USB serial driver running on a host PC::
  ...
  | LinaOS       |  Serial  | Driver      |--------
  | Operating   |  Driver  | and         |
  | System                   USB Stack   |
   --------------------------------------

On the device-side LinaOS system, the gadget serial driver looks
...
interface.  Gadget serial has been tested with the LinaOS ACM driver,
the Windows usbser.sys ACM driver, and the LinaOS USB generic serial
driver.
...
-----------------------------------
To use the gadget serial driver you must configure the LinaOS gadget
...
side LinaOS system.  You can add this to the start up scripts, if
desired.
...
either the Windows or LinaOS ACM driver on the host side.  If gadget
serial is loaded as a bulk in/out device, you will want to use the
LinaOS generic serial driver on the host side.  Follow the appropriate
instructions below to install the host side driver.


Installing the Windows Host ACM Driver
--------------------------------------
To use the Windows ACM driver you must have the "linaos-cdc-acm.inf"
...
driver in the folder that contains the "linaos-cdc-acm.inf" file.
...
path or browse to the folder containing the "linaos-cdc-acm.inf" file.
...
Installing the LinaOS Host ACM Driver
------------------------------------
To use the LinaOS ACM driver you must configure the LinaOS host side
...
to the LinaOS host with a USB cable, the host system should recognize
...
should show something like this:::
  ...
  S:  Manufacturer=LinaOS 2.6.8.1 with net2280
...
If the host side LinaOS system is configured properly, the ACM driver
should be loaded automatically.  The command "lsmod" should show the
"acm" module is loaded.


Installing the LinaOS Host Generic USB Serial Driver
---------------------------------------------------
To use the LinaOS generic USB serial driver you must configure the
LinaOS host side kernel for "Support for Host-side USB", for "USB
...
On a LinaOS host running the ACM driver, configure minicom similarly
but use "/dev/ttyACM0" as the "Serial Device".  (If you have other
ACM devices connected, change the device name appropriately.)

On a LinaOS host running the USB generic serial driver, configure
