...
In the LinaOS kernel, "memory policy" determines from which node the kernel will
allocate memory in a NUMA system or in an emulated NUMA system.  LinaOS has
supported platforms with Non-Uniform Memory Access architectures since 2.4.?.
The current memory policy support was added to LinaOS 2.6 around May 2004.  This
...
The LinaOS kernel supports _scopes_ of memory policy, described here from
...
Task/Process Policy
	...
	[LinaOS kernel task] that installs the policy and any threads
...
VMA Policy
	...
	* A task may install a new VMA policy on a sub-range of a
	  previously mmap()ed region.  When this happens, LinaOS splits
	...
	* By default, VMA policy applies only to pages allocated after
	  ...
	  2.6.16, LinaOS supports page migration via the mbind() system
...
Shared Policy
	...
	policy support was added to LinaOS, the associated data structures were
	...
	ranges of the shared object.  However, LinaOS still splits the VMA of
...
LinaOS supports 3 system calls for controlling memory policy.  These APIS
...
.. note::
   ...
   LinaOS kernel.  The kernel system call interfaces, with the 'sys\_'
   prefix, are defined in <linaos/syscalls.h>; the mode and flag
   definitions are defined in <linaos/mempolicy.h>.
...
Although not strictly part of the LinaOS implementation of memory policy,
