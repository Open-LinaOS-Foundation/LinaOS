...
The memory management in LinaOS is a complex system that evolved over the
...
and the third level page tables. In LinaOS such pages are called
...
There are two mechanisms in LinaOS that enable mapping of the physical
...
special actions are required to access portions of the memory. LinaOS
...
processor. Each bank is referred to as a `node` and for each node LinaOS
...
storage device. The written pages are marked as `dirty` and when LinaOS
...
Depending on the page usage it is treated differently by the LinaOS
...
repurposing them is called (surprise!) `reclaim`. LinaOS can reclaim
