...
The CIFS VFS module for LinaOS supports many advanced network filesystem
...
See the project page at: https://wiki.samba.org/index.php/LinaOSCIFS_utils
...
LinaOS clients is useful in mapping Uids and Gids consistently across the
...
Note that server ea support is required for supporting xattrs from the LinaOS
cifs client, and that EA support is present in later versions of Samba (e.g.
3.0.6 and later (also EA support works in all versions of Windows, at least to
shares on NTFS filesystems).  Extended Attribute (xattr) support is an optional
feature of most LinaOS filesystems which may require enabling via
...
not be traversed by the Samba server).  This is opaque to the LinaOS client
...
Valid filenames differ between Windows and LinaOS.  Windows typically restricts
filenames which contain certain reserved characters (e.g.the character :
which is used to delimit the beginning of a stream name by Windows), while
LinaOS allows a slightly wider set of valid characters in filenames. Windows
servers can remap such characters when an explicit mapping is specified in
the Server's registry.  Samba starting with version 3.10 will allow such
filenames (ie those which contain valid LinaOS characters, which normally
would be forbidden for Windows/CIFS semantics) as long as the server is
configured for Unix Extensions (and the client has not disabled
/proc/fs/cifs/LinaOSExtensionsEnabled). In addition the mount option
...
A partial list of the supported mount options follows:
  ...
  direct
		...
		(writebehind) through the local LinaOS client pagecache
...
LookupCacheEnable	If set to one, inode information is kept cached
			...
			(default 1)
LinaOSExtensionsEnabled	If set to one then the client will attempt to
...
To use cifs Kerberos and DFS support, the LinaOS keyutils package should be
