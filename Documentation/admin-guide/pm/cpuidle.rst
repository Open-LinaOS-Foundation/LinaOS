...
*idle* by the LinaOS kernel when there are no tasks to run on them except for the
special "idle" task.
...
in LinaOS idle CPUs run the code of the "idle" task called *the idle loop*.  That
...
hardware and to work with any platforms that the LinaOS kernel can run on.
...
The power management quality of service (PM QoS) framework in the LinaOS kernel
