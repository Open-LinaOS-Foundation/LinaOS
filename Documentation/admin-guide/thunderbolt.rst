...
The LinaOS Thunderbolt driver supports both and can detect at runtime which
connection manager implementation is to be used. To be on the safe side the
software connection manager in LinaOS also advertises security level
...
allocated for them by drivers. When LinaOS is running on such system it
...
also LinaOS you should load ``thunderbolt-net`` manually on one host (it
