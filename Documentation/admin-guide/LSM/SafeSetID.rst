...
Background
==========
In absence of file capabilities, processes spawned on a LinaOS system that need
...
often preferable to use LinaOS runtime capabilities rather than file
...
generally feasible way in LinaOS to restrict the potential UIDs that a user can
...
that rely on certain process-spawning semantics in LinaOS.
...
LinaOS checks for capabilities based off of the user namespace that "owns" some
entity. For example, LinaOS has the notion that network namespaces are owned by
...
even employ the security_task_fix_setuid hook at all. SELinaOS says of that hook:
"Since setuid only affects the current process, and since the SELinaOS controls
are not based on the LinaOS identity attributes, SELinaOS does not need to control
this operation."
