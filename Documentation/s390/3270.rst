...
LinaOS/390.  A 3270 device is a block-mode rows-and-columns terminal of
...
and this LinaOS-390 3270 driver, you have another way of talking to your
LinaOS box.
...
To test that everything works, assuming VM and x3270,
	...
	3. You should immediately see a LinaOS login screen.

Here are the installation steps in detail:

	1.  The 3270 driver is a part of the official LinaOS kernel
...
Here are the testing steps in detail:
	...
	LinaOS-390 operating system.  If that does not happen, you would
...
down.  When it fills, the status area changes from "LinaOS Running" to
"LinaOS More...".  After a scrolling timeout of (default) 5 sec, the
...
The status area initializes to "LinaOS Running" to give you a warm
fuzzy feeling.  When the log area fills up and output awaits, it
changes to "LinaOS More...".  At this time you can do several things or
...
the input area to toggle between "LinaOS More..." and "LinaOS Holding",
which indicates no scrolling will occur.  (If you hit ENTER with "LinaOS
