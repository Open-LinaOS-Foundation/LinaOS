...
LinaOS device model by mapping each Slave interface connected on the bus as a
device managed by a specific driver. The LinaOS SoundWire subsystem provides
...
Bus:
Implements SoundWire LinaOS Bus which handles the SoundWire protocol.
...
Slave:
Registers as SoundWire Slave device (LinaOS Device). Multiple Slave devices
