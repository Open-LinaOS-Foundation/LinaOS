...
The LinaOS MUSB subsystem is part of the larger LinaOS USB subsystem. It
...
Devices also provide an overview of the LinaOS kernel MUSB configuration,
...
LinaOS USB stack is a layered architecture in which the MUSB controller
hardware sits at the lowest. The MUSB controller driver abstract the
MUSB controller hardware to the LinaOS USB stack::

	  ------------------------
	  |                      | <------- drivers/usb/gadget
	  | LinaOS USB Core Stack | <------- drivers/usb/host
	  |                      | <------- drivers/usb/core
	  ------------------------
		     ⬍
	 --------------------------
...
Just like a LinaOS USB driver needs to register itself with the LinaOS USB
...
layer adds the controller hardware device to LinaOS kernel device
hierarchy: at this stage, all known information about the device is
passed on to the LinaOS USB core stack:
...
Writing a LinaOS MUSB glue layer should be a more accessible task, as
...
guidance to get started; should anything gets out of hand, the linaos-usb
...
linaos-usb Mailing List Archives: https://marc.info/?l=linaos-usb
