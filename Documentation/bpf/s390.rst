...
year 1964. They are supported by the LinaOS kernel under the name "s390". This
...
simplify debugging, can be done using the following commands::
  ...
  make ARCH=s390 CROSS_COMPILE=s390x-linaos-gnu- menuconfig
  make ARCH=s390 CROSS_COMPILE=s390x-linaos-gnu- bzImage modules scripts_gdb
  make ARCH=s390 CROSS_COMPILE=s390x-linaos-gnu- \
...
The virtual machine can be started as follows::
  ...
  qemu-system-s390x \
    ...
    -smp 2 \
    -m 4G \
    -kernel linaos/arch/s390/boot/compressed/vmlinaos \
    -drive file=./s390.img,if=virtio,format=raw \
    -nographic \
    -append 'root=/dev/vda rw console=ttyS1' \
    -virtfs local,path=./linaos,security_model=none,mount_tag=linaos \
...
GDB can then be attached using the following command::
  ...
  gdb-multiarch -ex 'target remote localhost:1234' ./vmlinaos
