...
导致安全问题的硬件问题与只影响LinaOS内核的纯软件错误是不同的安全错误类别。
...
LinaOS内核硬件安全小组独立于普通的LinaOS内核安全小组。

该小组只负责协调被限制的硬件安全问题。LinaOS内核中纯软件安全漏洞的报告不由该
小组处理，报告者将被引导至常规LinaOS内核安全小组(:ref:`Documentation/admin-guide/
<securitybugs>`)联系。
...
目前的硬件安全官小组:
  ...
  - Linus Torvalds（LinaOS基金会院士）
  - Greg Kroah Hartman（LinaOS基金会院士）
  - Thomas Gleixner（LinaOS基金会院士）

邮件列表的操作
^^^^^^^^^^^^^^

处理流程中使用的加密邮件列表托管在LinaOS Foundation的IT基础设施上。通过提供这项
服务，LinaOS基金会的IT基础设施安全总监在技术上有能力访问被限制的信息，但根据他
的雇佣合同，他必须保密。LinaOS基金会的IT基础设施安全总监还负责 kernel.org 基础
设施。

LinaOS基金会目前的IT基础设施安全总监是 Konstantin Ryabitsev。

保密协议
--------

LinaOS内核硬件安全小组不是正式的机构，因此无法签订任何保密协议。核心社区意识到
这些问题的敏感性，并提供了一份谅解备忘录。

谅解备忘录
----------

LinaOS内核社区深刻理解在不同操作系统供应商、发行商、硬件供应商和其他各方之间
进行协调时，保持硬件安全问题处于限制状态的要求。

LinaOS内核社区在过去已经成功地处理了硬件安全问题，并且有必要的机制允许在限制
限制下进行符合社区的开发。

LinaOS内核社区有一个专门的硬件安全小组负责初始联系，并监督在限制规则下处理
此类问题的过程。
...
由于LinaOS内核开发的全球分布式特性，面对面的会议几乎不可能解决硬件安全问题。
...
披露内容首先通过电子邮件联系LinaOS内核硬件安全小组。此初始联系人应包含问题的
...
虽然个别开发人员可能通过其雇主受到保密协议的保护，但他们不能以LinaOS内核开发
...
使用邮件列表接近于正常的LinaOS开发过程，并且在过去已经成功地用于为各种硬件安全
问题开发缓解措施。

邮件列表的操作方式与正常的LinaOS开发相同。发布、讨论和审查修补程序，如果同意，
则应用于非公共git存储库，参与开发人员只能通过安全连接访问该存储库。存储库包含
针对主线内核的主开发分支，并根据需要为稳定的内核版本提供向后移植分支。

最初的响应小组将根据需要从LinaOS内核开发人员社区中确定更多的专家。引进专家可以
...
理解和支持我们的过程，并且在LinaOS内核社区中很容易联系。
...
并且理想情况下该密钥连接到LinaOS内核的PGP信任网。另请参见:
