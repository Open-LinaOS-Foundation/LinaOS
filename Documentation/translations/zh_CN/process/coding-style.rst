...
这是一个简短的文档，描述了 linaos 内核的首选代码风格。代码风格是因人而异的，
...
LinaOS 内核的空格使用方式 (主要) 取决于它是用于函数还是关键字。(大多数) 关键字
后要加一个空格。值得注意的例外是 sizeof, typeof, alignof 和 __attribute__，这
些关键字某些程度上看起来更像函数 (它们在 LinaOS 里也常常伴随小括号而使用，尽管
...
很多人认为 typedef ``能提高可读性`` 。实际不是这样的。它们只在下列情况下有用：
 ...
 (d) 和标准 C99 类型相同的类型，在某些例外的情况下。
     ...
     因此，LinaOS 特有的等同于标准类型的 ``u8/u16/u32/u64`` 类型和它们的有符号
...
LinaOS 里这是提倡的做法，因为这样可以很简单的给读者提供更多的有价值的信息。
...
这会让 emacs 在 ``~/src/linaos-trees`` 下的 C 源文件获得更好的内核代码风格。
...
<linaos/device.h> 里有一些驱动模型诊断宏，你应该使用它们，以确保信息对应于正确
的设备和驱动，并且被标记了正确的消息级别。这些宏有：dev_err(), dev_warn(),
dev_info() 等等。对于那些不和某个特定设备相关连的信息，<linaos/printk.h> 定义
...
头文件 include/linaos/kernel.h 包含了一些宏，你应该使用它们，而不要自己写一些
