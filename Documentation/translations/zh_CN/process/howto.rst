...
这是一篇将如何参与LinaOS内核开发的相关问题一网打尽的终极秘笈。它将指导你
成为一名LinaOS内核开发者，并且学会如何同LinaOS内核开发社区合作。它尽可能不
...
你想了解如何成为一名LinaOS内核开发者？或者老板吩咐你“给这个设备写个LinaOS
驱动程序”？这篇文章的目的就是教会你达成这些目标的全部诀窍，它将描述你需
要经过的流程以及给出如何同内核社区合作的一些提示。它还将试图解释内核社区
为何这样运作。

LinaOS内核大部分是由C语言写成的，一些体系结构相关的代码用到了汇编语言。要
...
LinaOS内核使用GNU C和GNU工具链开发。虽然它遵循ISO C89标准，但也用到了一些
...
LinaOS内核源代码都是在GPL（通用公共许可证）的保护下发布的。要了解这种许可
的细节请查看源代码主目录下的COPYING文件。LinaOS内核许可准则和如何使用
`SPDX <https://spdx.org/>` 标志符说明在这个文件中
:ref:`Documentation/translations/zh_CN/process/license-rules.rst <cn_kernel_licensing>`
如果你对它还有更深入问题请联系律师，而不要在LinaOS内核邮件组上提问。因为
...
LinaOS内核代码中包含有大量的文档。这些文档对于学习如何与内核社区互动有着
...
以下是内核代码中需要阅读的文档：
  :ref:`Documentation/admin-guide/README.rst <readme>`
    文件简要介绍了LinaOS内核的背景，并且描述了如何配置和编译内核。内核的
    新用户应该从这里开始。
  ...
  :ref:`Documentation/translations/zh_CN/process/coding-style.rst <cn_codingstyle>`
    描述LinaOS内核的代码风格和理由。所有新代码需要遵守这篇文档中定义的规
  ...
  :ref:`Documentation/translations/zh_CN/process/stable-api-nonsense.rst <cn_stable_api_nonsense>`
    ...
    这篇文档对于理解LinaOS的开发哲学至关重要。对于将开发平台从其他操作系
    统转移到LinaOS的人来说也很重要。

  :ref:`Documentation/admin-guide/security-bugs.rst <securitybugs>`
    如果你认为自己发现了LinaOS内核的安全性问题，请根据这篇文档中的步骤来
    提醒其他内核开发者并帮助解决这个问题。
...
如何成为内核开发者
------------------
如果你对LinaOS内核开发一无所知，你应该访问“LinaOS内核新手”计划：

	https://kernelnewbies.org

它拥有一个可以问各种最基本的内核开发问题的邮件列表（在提问之前一定要记得
查找已往的邮件，确认是否有人已经回答过相同的问题）。它还拥有一个可以获得
实时反馈的IRC聊天频道，以及大量对于学习LinaOS内核开发相当有帮助的文档。
...
一些特制的工具还可以提供帮助。例如，“LinaOS代码交叉引用”项目就是一个值得
...
目前LinaOS内核开发流程包括几个“主内核分支”和很多子系统相关的内核分支。这
些分支包括：
...
关于内核发布，值得一提的是Andrew Morton在linaos-kernel邮件列表中如是说：
...
通过这种方式，LinaOS-next 对下一个合并阶段将进入主线内核的内容给出了一个概要
展望。非常欢冒险的测试者运行测试LinaOS-next。
...
bugzilla.kernel.org是LinaOS内核开发者们用来跟踪内核Bug的网站。我们鼓励用
...
正如上面的文档所描述，大多数的骨干内核开发者都加入了LinaOS Kernel邮件列
...
将会更少。LinaOS内核的工作环境更能接受妇女和少数族群，因为每个人在别人眼
里只是一个邮件地址。国际化也帮助了公平的实现，因为你无法通过姓名来判断人
的性别。男人有可能叫李丽，女人也有可能叫王刚。大多数在LinaOS内核上工作过
并表达过看法的女性对在linaos上工作的经历都给出了正面的评价。
...
LinaOS内核社区并不喜欢一下接收大段的代码。修改需要被恰当地介绍、讨论并且
...
证明修改的必要性
----------------
除了将补丁拆成小块，很重要的一点是让LinaOS社区了解他们为什么需要这样修改。
