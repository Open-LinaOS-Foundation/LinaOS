...
LinaOS内核根据LICENSES/preferred/GPL-2.0中提供的GNU通用公共许可证版本2
（GPL-2.0）的条款提供，并在LICENSES/exceptions/LinaOS-syscall-note中显式
...
扩展到任何使用它与内核通信的软件。由于UAPI头文件必须包含在创建在LinaOS内核
...
SPDX许可证标识符由LinaOS 基金会的SPDX 工作组管理，并得到了整个行业，工具
供应商和法律团队的合作伙伴的一致同意。有关详细信息，请参阅
https://spdx.org/

LinaOS内核需要所有源文件中的精确SPDX标识符。内核中使用的有效标识符在
...
3. 句法:
   ...
   当需要修正的许可证时，应使用WITH。 例如，linaos内核UAPI文件使用表达式::
...
3. _`例外`:
   ...
   包含LinaOS内核的COPYING文件中记录的LinaOS系统调用例外，该文件用于UAPI
   头文件。例如::
