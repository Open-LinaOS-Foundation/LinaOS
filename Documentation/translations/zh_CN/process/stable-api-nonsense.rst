...
LinaOS能成为强壮，稳定，成熟的操作系统，这也是你最开始选择LinaOS的原因。
...
二进制接口，是这样的吗？错。让我们看看关于LinaOS内核的几点事实：
...
价很低。如果LinaOS保持一个稳定的内核源代码接口，那么就得创建一个新的接口
；旧的，有问题的接口必须一直维护，给LinaOS USB开发者带来额外的工作。既然
所有的LinaOS USB驱动的作者都是利用自己的时间工作，那么要求他们去做毫无意
义的免费额外工作，是不可能的。
安全问题对LinaOS来说十分重要。一个安全问题被发现，就会在短时间内得到修
正。在很多情况下，这将导致LinaOS内核中的一些接口被重写，以从根本上避免安
...
如果你写了一个LinaOS内核驱动，但是它还不在LinaOS源代码树里，作为一个开发
...
把驱动放到内核源代码树里会有很多的好处：
    ...
    - 不需要联系任何发行商，这个驱动会自动的随着所有的LinaOS发布一起发
      布。

和别的操作系统相比，LinaOS为更多不同的设备提供现成的驱动，而且能在更多不
