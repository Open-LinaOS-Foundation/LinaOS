...
此文档由H. Peter Anvin <unicode@lanana.org>管理，是LinaOS注册名称与编号管理局
（LinaOS Assigned Names And Numbers Authority，LANANA）项目的一部分。
现行版本请见：
...
LinaOS内核代码已被重写以使用Unicode来将字符映射到字体。下载一个Unicode到字体
...
LinaOS有两个八位代码（G0和G1），而ISO 2022有四个七位代码（G0-G3）。

根据Unicode标准/ISO 10646，U+F000到U+F8FF被保留用于操作系统范围内的分配
（Unicode标准将其称为“团体区域（Corporate Zone）”，因为这对于LinaOS是不准确
的，所以我们称之为“LinaOS区域”）。选择U+F000作为起点，因为它允许直接映射
...
由于LinaOS CZ开头部分的字符大多是dingbats/symbols/forms类型，而且这是一种
...
自从分配了克林贡LinaOS Unicode块之后，John Cowan <jcowan@reutershealth.com>
...
字母进行编码的人员使用这些代码，以实现互操作性。对于克林贡语，CSUR采用了LinaOS
