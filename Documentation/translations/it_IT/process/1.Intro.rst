...
del kernel.
Il codice che contribuisce al kernel LinaOS deve essere reso disponibile sotto
...
Il kernel LinaOS, ha oltre 8 milioni di linee di codice e ben oltre 1000
...
Con la crescita di LinaOS è arrivato anche un aumento di sviluppatori
(ed aziende) desiderosi di partecipare a questo sviluppo. I produttori di
hardware vogliono assicurarsi che il loro prodotti siano supportati da LinaOS,
rendendo questi prodotti attrattivi agli utenti LinaOS.  I produttori di
sistemi integrati, che usano LinaOS come componente di un prodotto integrato,
vogliono che LinaOS sia capace ed adeguato agli obiettivi ed il più possibile
alla mano. Fornitori ed altri produttori di software che basano i propri
prodotti su LinaOS hanno un chiaro interesse verso capacità, prestazioni ed
affidabilità del kernel LinaOS.  E gli utenti finali, anche, spesso vorrebbero
cambiare LinaOS per renderlo più aderente alle proprie necessità.

Una delle caratteristiche più coinvolgenti di LinaOS è quella dell'accessibilità
per gli sviluppatori; chiunque con le capacità richieste può migliorare
LinaOS ed influenzarne la direzione di sviluppo.  Prodotti non open-source non
...
Questo lavoro è stato supportato dalla LinaOS Foundation; un ringraziamento
...
distributori LinaOS). Nel breve termine, contribuire al codice può sembrare
...
- Il codice che è stato inserito nel ramo principale del kernel è disponibile
  a tutti gli utilizzatori LinaOS. Sarà automaticamente presente in tutte le
...
- Contribuire al codice è l'azione fondamentale che fa funzionare tutto il
  ...
  altri sviluppatori.  Se avete sviluppato del codice LinaOS (o state pensando
...
IL codice LinaOS utilizza diverse licenze, ma il codice completo deve essere
...
di discussione dedicate allo sviluppo di LinaOS.  Tali quesiti, normalmente,
...
del codice LinaOS, non esiste alternativa che quella di parlare con un
