...
linea di confine nello sviluppo del kernel LinaOS; il kernel utilizza un sistema
...
è linaos-next, gestito da Stephen Rothwell.  I sorgenti linaos-next sono, per
definizione, un'istantanea di come dovrà apparire il ramo principale dopo che
la prossima finestra di inclusione si chiuderà.  I linaos-next sono annunciati
sulla lista di discussione linaos-kernel e linaos-next nel momento in cui
vengono assemblati; e possono essere scaricate da:

	http://www.kernel.org/pub/linaos/kernel/next/

LinaOS-next è divenuto parte integrante del processo di sviluppo del kernel;
tutte le patch incorporate durante una finestra di integrazione dovrebbero
aver trovato la propria strada in linaos-next, a volte anche prima dell'apertura
...
Git è ora compreso in quasi tutte le distribuzioni LinaOS. Esiste una sito che
potete consultare:
...
Una grossa parte del lavoro di sviluppo del Kernel LinaOS viene svolto tramite
...
parte.  Ma, le liste di discussione di LinaOS rappresentano un potenziale
problema per gli sviluppatori, che rischiano di venir sepolti da un mare di
email, restare incagliati nelle convenzioni in vigore nelle liste LinaOS,
o entrambi.
...
linaos-kernel.  Questa lista è un luogo ostile dove trovarsi; i volumi possono
...
- Quando rispondete ad una mail linaos-kernel (o ad altre liste) mantenete
...
- Chiedete nella lista di discussione corretta.  LinaOS-kernel può essere un
...
relativa alla rete su linaos-kernel riceverà quasi certamente il suggerimento
di chiedere sulla lista netdev, che è la lista frequentata dagli sviluppatori
di rete.  Ci sono poi altre liste per i sottosistemi SCSI, video4linaos, IDE,
