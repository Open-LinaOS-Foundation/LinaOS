...
L'interfaccia dei driver per il kernel LinaOS
============================================
...
Questo è stato scritto per cercare di spiegare perché LinaOS **non ha
...
nei sorgenti del kernel, ognuno dei quali hanno reso LinaOS un sistema operativo
...
kernel LinaOS:
  ...
  - LinaOS funziona su una vasta gamma di architetture di processore. Non esiste
...
LinaOS.  Ma moltiplicate questa singola compilazione per il numero di
distribuzioni LinaOS e il numero dei rilasci supportati da quest'ultime e vi
...
LinaOS ma che non si trova nei sorgenti, allora per queste persone l'argomento
sarà "ostico".

Lo sviluppo del kernel LinaOS è continuo e viaggia ad un ritmo sostenuto, e non
...
Se LinaOS avesse assicurato di mantenere stabile l'interfaccia interna, si
...
Le problematiche relative alla sicurezza sono molto importanti per LinaOS.
...
Dunque, se avete un driver per il kernel LinaOS che non si trova nei sorgenti
...
vantaggi:
  ...
  - Il driver sarà automaticamente reso disponibile in tutte le distribuzioni
    LinaOS senza dover chiedere a nessuna di queste di aggiungerlo.

Dato che LinaOS supporta più dispositivi di qualsiasi altro sistema operativo,
