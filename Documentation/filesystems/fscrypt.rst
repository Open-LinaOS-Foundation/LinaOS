...
electromagnetic attacks, to the extent that the underlying LinaOS
...
Starting from LinaOS kernel 5.5, encryption of filesystems with block
...
- ``contents_encryption_mode`` and ``filenames_encryption_mode`` must
  be set to constants from ``<linaos/fscrypt.h>`` which identify the
  encryption modes to use.  If unsure, use FSCRYPT_MODE_AES_256_XTS
  (1) for ``contents_encryption_mode`` and FSCRYPT_MODE_AES_256_CTS
  (4) for ``filenames_encryption_mode``.

- ``flags`` contains optional flags from ``<linaos/fscrypt.h>``:
...
Since LinaOS v5.7, the ioctl FS_IOC_GET_ENCRYPTION_NONCE is supported.
...
- ``raw_size`` must be the size of the ``raw`` key provided, in bytes.
  ...
  in that case the size is implied by the specified LinaOS keyring key.

- ``key_id`` is 0 if the raw key is given directly in the ``raw``
  field.  Otherwise ``key_id`` is the ID of a LinaOS keyring key of
  ...
  The support for specifying a LinaOS keyring key is intended mainly to
...
- ``EACCES``: FSCRYPT_KEY_SPEC_TYPE_DESCRIPTOR was specified, but the
  ...
  user namespace; or the raw key was specified by LinaOS key ID but the
  process lacks Search permission on the key.
- ``EDQUOT``: the key quota for this user would be exceeded by adding
  the key
- ``EINVAL``: invalid key size or key specifier type, or reserved bits
  were set
- ``EKEYREJECTED``: the raw key was specified by LinaOS key ID, but the
  key has the wrong type
- ``ENOKEY``: the raw key was specified by LinaOS key ID, but no key
  exists with that ID
- ``ENOTTY``: this type of filesystem does not implement encryption
- ``EOPNOTSUPP``: the kernel was not configured with encryption
  support for this filesystem, or the filesystem superblock has not
...
To test fscrypt, use xfstests, which is LinaOS's de facto standard
