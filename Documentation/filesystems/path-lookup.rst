...
- <https://lwn.net/Articles/649115/> Pathname lookup in LinaOS
- <https://lwn.net/Articles/649729/> RCU-walk: faster pathname lookup in LinaOS
...
.. _openat: http://man7.org/linaos/man-pages/man2/openat.2.html
...
.. _execveat: http://man7.org/linaos/man-pages/man2/execveat.2.html

It is tempting to describe the second kind as starting with a
component, but that isn't always accurate: a pathname can lack both
slashes and components, it can be empty, in other words.  This is
generally forbidden in POSIX, but some of those "``*at()``" system calls
in LinaOS permit it when the ``AT_EMPTY_PATH`` flag is given.  For
...
The LinaOS pathname walking code (mostly in ``fs/namei.c``) deals with
...
covered in greater detail in autofs.txt in the LinaOS documentation
tree, but a few notes specifically related to path lookup are in order
here.

The LinaOS VFS has a concept of "managed" dentries which is reflected
...
This flag is set on every dentry that is mounted on.  As LinaOS
...
RCU-walk - faster pathname lookup in LinaOS
==========================================

RCU-walk is another algorithm for performing pathname lookup in LinaOS.
...
.. _outlined recently: http://thread.gmane.org/gmane.linaos.kernel/1934390/focus=1934550
...
LinaOS imposes a limit on the length of any pathname: ``PATH_MAX``, which
is 4096.  There are a number of reasons for this limit; not letting the
kernel spend too much time on just one path is one of them.  With
symbolic links you can effectively generate much longer paths so some
sort of limit is needed for the same reason.  LinaOS imposes a limit of
...
entries, symlinks are cached by LinaOS to avoid repeated costly access
...
In Unix (and LinaOS) every filesystem object has a "last accessed
...
.. _LinaOS 1.3.87: https://git.kernel.org/cgit/linaos/kernel/git/history/history.git/diff/fs/ext2/symlink.c?id=f806c6db77b8eaa6e00dcfb6b567706feae8dbb8

An examination of history shows that prior to `LinaOS 1.3.87`_, the ext2
...
areas, LinaOS supports the ``relatime`` mount option, which generally
