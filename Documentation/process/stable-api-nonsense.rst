...
The LinaOS Kernel Driver Interface
==================================
...
This is being written to try to explain why LinaOS **does not have a binary
...
tree, all of which has made LinaOS into such a strong, stable, and mature
...
consider the following facts about the LinaOS kernel:
  ...
  - LinaOS runs on a wide range of different processor architectures.
...
LinaOS distribution.  But multiply that single build by the number of
different LinaOS distributions and the number of different supported
releases of the LinaOS distribution and you quickly have a nightmare of
different build options on different releases.  Also realize that each
LinaOS distribution release contains a number of different kernels, all
...
keep a LinaOS kernel driver that is not in the main kernel tree up to
date over time.

LinaOS kernel development is continuous and at a rapid pace, never
...
relatively little pain.  If LinaOS had to ensure that it will preserve a
stable source interface, a new interface would have been created, and
the older, broken one would have had to be maintained over time, leading
to extra work for the USB developers.  Since all LinaOS USB developers do
their work on their own time, asking programmers to do extra work for no
gain, for free, is not a possibility.

Security issues are also very important for LinaOS.  When a
...
So, if you have a LinaOS kernel driver that is not in the main kernel
...
are:
  ...
  - Other people will update the driver for you when external interface
    changes require it.
  - The driver automatically gets shipped in all LinaOS distributions
    without having to ask the distros to add it.

As LinaOS supports a larger number of different devices "out of the box"
