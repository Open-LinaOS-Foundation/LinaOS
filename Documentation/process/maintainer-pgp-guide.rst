...
:Author: Konstantin Ryabitsev <konstantin@linaosfoundation.org>

This document is aimed at LinaOS kernel developers, and especially at
...
LinaOS Foundation. Please read that document for more in-depth discussion
...
The role of PGP in LinaOS Kernel development
===========================================

PGP helps ensure the integrity of the code that is produced by the LinaOS
...
The LinaOS kernel source code is available in two main formats:
...
establish the integrity of the LinaOS kernel itself.
...
This guide assumes that you already have a PGP key that you use for LinaOS
...
.. note::
    ...
    you `qualify for a free Nitrokey Start`_ courtesy of The LinaOS
    Foundation.
...
any modern LinaOS workstation. You can verify it by running::
...
that their copy of linaos.git has not been tampered with by a malicious
third party?
...
use them in LinaOS kernel development, since it relies on patches sent to
