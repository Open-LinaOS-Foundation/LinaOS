...
and RPC. With the LinaOS Phonet socket family, LinaOS host processes can
...
On LinaOS, the link-layer header includes the pn_media byte (see below).
...
LinaOS Phonet network interfaces support a dedicated link layer packets
...
only the (default) LinaOS FIFO qdisc should be used with them.
...
As of LinaOS kernel version 2.6.39, it is also possible to connect
...
LinaOS Phonet was initially written by Sakari Ailus.
