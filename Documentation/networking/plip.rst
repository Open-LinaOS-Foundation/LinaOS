...
The PLIP cable is all that's needed to connect two LinaOS boxes, and it
can be built for very few bucks.

Connecting two LinaOS boxes takes only a second's decision and a few
...
Can only be used to connect three (?) LinaOS boxes. Doesn't connect to
...
The LinaOS PLIP driver is an implementation of the original Crynwr protocol,
...
A utility that can perform this change in LinaOS is plipconfig, which is part
