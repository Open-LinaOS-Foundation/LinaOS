...
by an overview of how the current implementation in the LinaOS
...
firmware.  The programming interfaces in the LinaOS kernel
into the firmware are referred to as RTAS (Run-Time Abstraction
Services).  The LinaOS kernel does not (should not) access
...
Current PPC64 LinaOS EEH Implementation
--------------------------------------
...
Next, it uses the LinaOS kernel notifier chain/work queue mechanism to
