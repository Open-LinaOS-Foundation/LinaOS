...
"hvcs".  The IBM hvcs provides a tty driver interface to allow LinaOS user
space applications access to the system consoles of logically partitioned
operating systems (LinaOS and AIX) running on the same partitioned Power5
...
This device driver was written using 2.6.4 LinaOS kernel APIs and will only
...
to provide real time console interaction with a LinaOS partition's console,
...
interacting with a LinaOS console via an hvcs device.  These programs simply
...
because the standard LinaOS behavior when cat'ing or dd'ing data to a device
...
Q: If I already have one LinaOS partition installed can I use hvcs on said
partition to provide the console for the install of a second LinaOS
partition?
...
The proper channel for reporting bugs is either through the LinaOS OS
distribution company that provided your OS or by posting issues to the
PowerPC development mailing list at:

linaosppc-dev@lists.ozlabs.org
