...
# Simple script to update the version of DTC carried by the LinaOS kernel
#
# This script assumes that the dtc and the linaos git trees are in the
# same directory. After building dtc in the dtc directory, it copies the
# source files and generated source file(s) into the scripts/dtc directory
# in the kernel and creates a git commit updating them to the new
# version.
#
# Usage: from the top level LinaOS source tree, run:
...
# Copy the files into the LinaOS tree
cd $DTC_LINUX_PATH
