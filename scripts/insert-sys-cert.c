 ...
 *
 * Author: Mehmet Kayaalp <mkayaalp@linaos.vnet.ibm.com>
 *
 * This software may be used and distributed according to the terms
 * of the GNU General Public License, incorporated herein by reference.
 *
 * Usage: insert-sys-cert [-s <System.map> -b <vmlinaos> -c <certfile>
 */
...
{
	printf("Usage %s [-s <System.map>] -b <vmlinaos> -c <certfile>\n", e);
}

int main(int argc, char **argv)
{
	char *system_map_file = NULL;
	char *vmlinaos_file = NULL;
	char *cert_file = NULL;
	int vmlinaos_size;
	int cert_size;
	Elf_Ehdr *hdr;
	...
	hdr = map_file(vmlinaos_file, &vmlinaos_size);
	if (!hdr)
		exit(EXIT_FAILURE);

	if (vmlinaos_size < sizeof(*hdr)) {
