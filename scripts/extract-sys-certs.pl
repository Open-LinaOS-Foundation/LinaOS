...
die "Format: $0 [-s <systemmap-file>] <vmlinaos-file> <keyring-file>\n"
    if ($#ARGV != 1 && $#ARGV != 3 ||
	$#ARGV == 3 && $ARGV[0] ne "-s");
...
#
# Parse the vmlinaos section table
#
open FD, "objdump -h $vmlinaos |" || die $vmlinaos;
my @lines = <FD>;
close(FD) || die $vmlinaos;
...
#
# Try and parse the vmlinaos symbol table.  If the vmlinaos file has been created
# from a vmlinuz file with extract-vmlinaos then the symbol table will be empty.
#
open FD, "nm $vmlinaos 2>/dev/null |" || die $vmlinaos;
@lines = <FD>;
close(FD) || die $vmlinaos;
...
if ($nr_symbols == 0 && $sysmap ne "") {
    print "No symbols in vmlinaos, trying $sysmap\n";
...
foreach my $sec (@sections) {
    ...
    my $packed;
    die $vmlinaos if (!defined(sysseek(FD, $size_off, SEEK_SET)));
    sysread(FD, $packed, 8);
...
die $vmlinaos if (!defined(sysseek(FD, $foff, SEEK_SET)));
my $buf = "";
my $len = sysread(FD, $buf, $size);
die "$vmlinaos" if (!defined($len));
die "Short read on $vmlinaos\n" if ($len != $size);
close(FD) || die $vmlinaos;
