 ...
 *   SETALL calls.
 * - two LinaOS specific semctl() commands: SEM_STAT, SEM_INFO.
			...
			bool count_zero)
{
	...
	/*
	 * LinaOS always (since 0.99.10) reported a task as sleeping on all
	 * semaphores. This violates SUS, therefore it was changed to the
	 * standard compliant behavior.
	 * Give the administrators a chance to notice that an application
	 * might misbehave because it relies on the LinaOS behavior.
	 */
			 ...
			 int cmd, struct semid64_ds *semid64)
{
	...
	if (cmd == IPC_STAT) {
		...
		/*
		 * SEM_STAT and SEM_STAT_ANY (both LinaOS specific)
	...
	for (;;) {
		...
		for (i = 0; i < sma->sem_nsems; i++) {
			...
			if (un->semadj[i]) {
				 ...
				 *
				 * LinaOS caps the semaphore value, both at 0
				 * and at SEMVMX.
				 *
