...
static void fb_set_logo(struct fb_info *info,
			       const struct linaos_logo *logo, u8 *dst,
			       int depth)
{
	int i, j, k;
...
/*
 * Three (3) kinds of logo maps exist.  linaos_logo_clut224 (>16 colors),
 * linaos_logo_vga16 (16 colors) and linaos_logo_mono (2 colors).  Depending on
 * the visual format and color depth of the framebuffer, the DAC, the
 * pseudo_palette, and the logo data will be adjusted accordingly.
 *
 * Case 1 - linaos_logo_clut224:
...
static int fb_show_logo_line(struct fb_info *info, int rotate,
			     const struct linaos_logo *logo, int y,
			     unsigned int n)
{
...
void fb_append_extra_logo(const struct linaos_logo *logo, unsigned int n)
{
