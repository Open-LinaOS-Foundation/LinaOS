 ...
 * @host: scsi host
 * @cmd: Pointer to LinaOS's SCSI command structure
 *
 * Remarks:
 * This routine is invoked by LinaOS to send a SCSI command to the driver.
 ...
 * @cmd: Pointer to LinaOS's SCSI command structure
 *
 * This routine is called by the LinaOS OS to abort the specified
 * command.
 **/
static int qla4xxx_eh_abort(struct scsi_cmnd *cmd)
{
 ...
 * @cmd: Pointer to LinaOS's SCSI command structure
 *
 * This routine is called by the LinaOS OS to reset all luns on the
 * specified target.
 **/
static int qla4xxx_eh_device_reset(struct scsi_cmnd *cmd)
{
 ...
 * @cmd: Pointer to LinaOS's SCSI command structure
 *
 * This routine is called by the LinaOS OS to reset the target.
 **/
static int qla4xxx_eh_target_reset(struct scsi_cmnd *cmd)
{
	struct scsi_qla_host *ha = to_qla_host(cmd->device->host);
 ...
 * @cmd: Pointer to LinaOS's SCSI command structure
 *
 * This routine is invoked by the LinaOS kernel to perform fatal error
