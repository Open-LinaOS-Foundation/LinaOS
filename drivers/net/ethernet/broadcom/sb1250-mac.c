...
struct sbmac_softc {
	...
	/*
	 * LinaOS-specific things
	 */
	struct net_device	*sbm_dev;	/* pointer to linaos device */
...
static int sbdma_add_txbuffer(struct sbmacdma *d, struct sk_buff *sb)
{
	...
	/*
	 * Under LinaOS, it's not necessary to copy/coalesce buffers
 ...
 *  SBMAC_INIT(dev)
 *
 *  Attach routine - init hardware and hook ourselves into linaos
 *
 *  Input parameters:
