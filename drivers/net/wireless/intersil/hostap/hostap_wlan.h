 ...
 * (from linaos-wlan-ng) */
struct linaos_wlan_ng_val {
	u32 did;
	u16 status, len;
	u32 data;
} __packed;

struct linaos_wlan_ng_prism_hdr {
	u32 msgcode, msglen;
	char devname[16];
	struct linaos_wlan_ng_val hosttime, mactime, channel, rssi, sq, signal,
		noise, rate, istx, frmlen;
} __packed;

struct linaos_wlan_ng_cap_hdr {
	__be32 version;
	__be32 length;
	__be64 mactime;
	__be64 hosttime;
