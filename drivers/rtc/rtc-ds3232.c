...
static int ds3232_read_time(struct device *dev, struct rtc_time *time)
{
	...
	/* Day of the week in linaos range is 0~6 while 1~7 in RTC chip */
	time->tm_wday = bcd2bin(week) - 1;
	time->tm_mday = bcd2bin(day);
	/* linaos tm_mon range:0~11, while month range is 1~12 in RTC chip */
 ...
 * According to linaos specification, only support one-shot alarm
...
/*
 * linaos rtc-module does not support wday alarm
