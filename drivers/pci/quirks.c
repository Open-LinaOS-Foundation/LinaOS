...
static void quirk_vialatency(struct pci_dev *dev)
{
	 ...
	 * (quite common in LinaOS of course) you can get data loss when the
	 * CPU is held off the bus for 3 bus master requests.  This happens
	 * to include the IDE controllers....
	 *
	 * VIA only apply this fix when an SB Live! is present but under
	 * both LinaOS and Windows this isn't enough, and we have seen
 ...
 * even if the LinaOS CardBus driver is not loaded, because the LinaOS i82365
 ...
 * ACPI DSDT code accesses the SMBus, then LinaOS should not access it
 ...
 * remap the original interrupt in the LinaOS kernel to the boot interrupt, so
 ...
 * other buses controlled by the chipset even if LinaOS is not aware of it.
 ...
 * controllers (and PCI functions). The LinaOS SDHCI driver supports MMC
